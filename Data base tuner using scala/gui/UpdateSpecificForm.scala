...
class MyModel6(var cells: Array[Array[Any]], val columns: Seq[Any]) extends AbstractTableModel {
  ...
  override def isCellEditable(row: Int, column: Int) = if(column == 1 && row != 0) true else  false;
...
class UpdateSpecificForm(data:Array[Array[Any]],  col:Seq[Any],vecField:Vector[Any],t:String,tName:String) extends MainFrame {
  ...
  background = Color.BLUE
   ...
   val table = new Table(data, col) {
    ...
    val mymodel = new MyModel6(data, col) {
  ...
  reactions += {
    case ButtonClicked(component) if component == btnEnter =>
     ...
     var check = srv.updateInSpecific(tableName, vecField, vecValue);
      if (check == true) {
        Dialog.showMessage(contents.head, "Succseesfully update the data", title = "You pressed me")
      } else {
        Dialog.showMessage(contents.head, "Failed to update ", title = "You pressed me")
      }
      this.dispose();
  }
